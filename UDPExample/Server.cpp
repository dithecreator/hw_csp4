	...
	while (true) {
		...
		for (int i = 0; i < MAX_CLIENTS; i++)
		{
			...
			if (FD_ISSET(s, &readfds))
			{
				...
				// Извлечь все переменные из полученного сообщения
				int color_code;
				char user_name[DEFAULT_BUFLEN];
				char message[DEFAULT_BUFLEN];
				sscanf_s(client_message, "%d|%[^|]|%[^\n]", &color_code, user_name, DEFAULT_BUFLEN, message, DEFAULT_BUFLEN);
				...
				case 3:
					...
					break;
				default:
					cout << "incorrect choice, white color will be used" << endl;
					textAttribute = FOREGROUND_GREEN | FOREGROUND_BLUE | FOREGROUND_RED;
					break;
				}
