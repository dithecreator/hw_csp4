...
DWORD WINAPI Sender(void* param)
{
    ...
    char name[DEFAULT_BUFLEN]; //переменная для получения имени
    ...
    while (true) {
        ...
        // Объединяем имя и запрос
        char fullNameAndQuery[2 * DEFAULT_BUFLEN]; // Результирующая строка

 


        //strcpy_s(fullNameAndQuery, DEFAULT_BUFLEN, name); // Копируем имя в результирующую строку
        //strcat_s(fullNameAndQuery, DEFAULT_BUFLEN, ": "); // Добавляем разделитель между именем и запросом
        //strcat_s(fullNameAndQuery, DEFAULT_BUFLEN, query); // Добавляем запрос в результирующую строку
